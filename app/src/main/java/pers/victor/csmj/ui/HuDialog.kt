...
class HuDialog(activity: Activity, val onClick: (Hu) -> Unit) : AlertDialog(activity) {
    ...
    fun setPeople(people: List<People>) {
        ...
        v.rg_hu.setOnCheckedChangeListener { _, id -> hu.winner = people[huIds.indexOf(id)] }
        ...
        v.rg_way.setOnCheckedChangeListener { _, id -> hu.loser = people[wayIds.indexOf(id)] }

        val bird1Ids = v.rg_bird1.children.map { it.id }
        v.rg_bird1.setOnCheckedChangeListener { _, id -> hu.birds[0] = people[bird1Ids.indexOf(id)] }

        val bird2Ids = v.rg_bird2.children.map { it.id }
        v.rg_bird2.setOnCheckedChangeListener { _, id -> hu.birds[1] = people[bird2Ids.indexOf(id)] }

        val dealerIds = v.rg_dealer.children.map { it.id }
        v.rg_dealer.setOnCheckedChangeListener { _, id -> hu.dealer = people[dealerIds.indexOf(id)] }
    }
}